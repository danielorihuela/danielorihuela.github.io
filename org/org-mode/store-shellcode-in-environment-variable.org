...
#+description: Learn how to store the shellcode in an environment variable and bypass size limitations in buffer overflows.
#+publishdate: 2023-10-29
#+cover: ../../images/covers/shellcode-env-var.jpg
#+options: ^:nil


In the [[../stack-based-buffer-overflows][previous post]] we explained how stack-based buffer overflows work. In the last exercise, we inserted the shellcode in the stack. However, this might be a problem. What happens if the shellcode doesn't fit in the stack? We can store it in an environment variable.
